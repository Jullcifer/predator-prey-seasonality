...
def bifurcationdiagram(parent_directory, saveplot=True):
    """
    ...
    """
    simulation_data = classify_and_read_data(parent_directory)
    ...
    np.savetxt(os.path.join(parent_directory, "category_matrix.txt"), category_matrix, fmt='%.0f', delimiter=', ')
    if saveplot:
        plot_bifurcation_diagram(aS_unique, nu_unique, category_matrix, f"{parent_directory}/BifurcationGridPlot.png")