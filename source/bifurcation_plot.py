...
def plot_bifurcation_diagram(aS_unique, nu_unique, category_matrix, save_path=None):
    """
    This function uses the results from the classification to plot the 
    bifurcation diagram. 

    Args:
        aS_unique (array): Unique values of aS.
    ...
    plt.pcolormesh(aS_unique, nu_unique, category_matrix, cmap=cmap_categories, shading='auto', vmin=0, vmax=len(CATEGORY_LABELS) - 1)
    # Add colorbar
    cbar = plt.colorbar()
